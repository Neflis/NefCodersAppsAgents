...
class FileTool:
    ...
    def __init__(
        ...
        max_file_size_bytes: int = 64 * 1024,
    ) -> None:
    ...
    def append_file(self, path: str | Path, content: str) -> str:
        ...
        if new_size > self.max_file_size_bytes:
            raise FileToolError("File would exceed the maximum allowed size.")
        safe_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _validate_extension(self, path: Path) -> None:
        if path.suffix not in self.allowed_extensions:
            raise FileToolError(f"Extension not allowed: {path.suffix}")

    def _validate_content_size(self, content: str) -> None:
        if len(content.encode("utf-8")) > self.max_file_size_bytes:
            raise FileToolError("Content exceeds the maximum allowed size.")
